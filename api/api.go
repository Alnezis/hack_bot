...
func Bs(buttons []k.Button, back []k.Button) vk.H {
	l := len(buttons)
	params := vk.H{}
	if l <= 2 {
		params = k.New(false, buttons, back)
	} else if l <= 4 {
		params = k.New(false, buttons[l/2:], buttons[:l/2], back)
	}else if l <= 6 {
		params = k.New(false, buttons[:2], buttons[2:4] , buttons[4:], back)
	}else if l <= 8 {
		params = k.New(false, buttons[:2], buttons[2:4] , buttons[4:6] , buttons[6:], back)
	}else if l <= 10 {
		params = k.New(false, buttons[:2], buttons[2:4] , buttons[4:6] , buttons[6:8],  buttons[8:], back)
	}else {
		params = k.New(false, buttons[:2], buttons[2:4] , buttons[4:6] , buttons[6:8],  buttons[8:10],  buttons[10:12], back)
	}
	return params
}
